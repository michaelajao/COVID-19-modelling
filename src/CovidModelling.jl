...
A module for processing and visualizing COVID-19 data.
...
plot_covid_cases(data::DataFrame, country_name::String, save_path::String)

Plot the COVID-19 cases for a given country.
...
julia> plot_covid_cases(filtered_data, "US")
...
This module provides three functions to process and visualize COVID-19 data. The data_processing function reads the confirmed cases, recovered, and death data for all countries from CSV files, selects the row corresponding to the specified country, extracts the date strings and converts them to Date objects, and returns a DataFrame containing the confirmed cases, recovered and death and dates for the specified country.

The filter_data_by_date function filters a DataFrame by date, selecting rows with dates greater than or equal to the start date and less than or equal to the end date. The function returns a new DataFrame containing the filtered data.

The plot_covid_cases function plots the confirmed COVID-19 cases for a given country, and saves the plot to a file. The function takes a DataFrame containing the COVID-19 data, the name of the country to be plotted, and the path to the file where the plot should be saved.
